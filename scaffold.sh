...
# Add database migration prompt
echo ""
read -p "Would you like to set up the initial database migration? (Y/n) " SETUP_DB
SETUP_DB=${SETUP_DB:-Y}
if [[ $SETUP_DB =~ ^[Yy]$ ]]; then
  echo "Please review and edit .env.example with your database credentials before we create .env"
  ...
  if [ -f ".env.example" ]; then
    ...
    npx prisma migrate dev --name "initialized the database"
