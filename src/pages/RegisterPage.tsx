...
function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast.success(registerResponse.message || "Registration successful! Please sign in.");
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full h-full p-5 flex flex-col gap-4 items-stretch justify-center">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Tell us a bit about yourself to get started! Please sign up and join the love ❤️</p>
      </div>
      ...
      <div className="w-full flex flex-col md:flex-row gap-4 items-center justify-between">
        <Button type="submit" className="md:flex-1 w-full md:w-fit">
          {register.isPending ? <Loader2 className="animate-spin">Loading...</Loader2> : "Sign Up"}
        </Button>
        <Button onClick={() => navigate("/auth/signin")} type="button" className="md:flex-1 w-full md:w-fit" variant="secondary">
          Sign In
        </Button>
      </div>
    </form>
